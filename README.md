...
| `PUSH_CATEGORY` | 条件必填 | - | 版块直推模式。设为 `all` 推送所有版块，或设为英文逗号分隔的版块标识如 `trade,daily`。命中指定版块时直接推送（不命中屏蔽词即可），未命中时仍走关键词匹配 |
...
未命中屏蔽词时，满足以下任一条件即推送：属于 `PUSH_CATEGORY` 指定的版块，或普通关键词任意命中、组合规则全词命中、正则命中。
...
- 优先级策略：屏蔽词 > 版块过滤 > PUSH_CATEGORY 版块匹配 > 关键词 > 组合词 > 正则，命中即推送；PUSH_CATEGORY 未命中时继续尝试关键词匹配
