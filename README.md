...
| `PUSH_CATEGORY` | 条件必填 | - | 版块直推模式。设为 `all` 推送所有版块，或设为英文逗号分隔的版块标识如 `trade,daily`。此模式下不命中屏蔽词即推送，无需关键词 |
...
- 优先级策略：屏蔽词 > 版块过滤 > 关键词 > 组合词 > 正则，命中即推送；设置 `PUSH_CATEGORY` 后跳过关键词/组合词/正则，版块匹配且不命中屏蔽词即推送
