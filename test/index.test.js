...
test('启动配置摘要包含版本、昵称、轮询间隔、匹配范围、版块、规则详情和自检间隔', () => {
  ...
  }), '版本 v1.6.0，MeoW 昵称 tester，轮询间隔 5 秒，匹配范围 all，监控版块 trade,daily，规则 4 条（关键词：VPS,优惠 | 组合词：香港+VPS | 正则：年付 | 版块匹配：trade），屏蔽词：求购，版块过滤：trade,daily，自检间隔 60 分钟');
});
...
test('启动时自检 RSS 连接并推送自检通知', async () => {
  ...
  await runApp({
    ...
    fetchItems: async () => [],
    checkUpdateFn: async (logger) => { logger.info('当前已是最新版本 v1.6.0'); return null; },
  ...
  assert.equal(logs[0], '启动配置：版本 v1.6.0，MeoW 昵称 tester，轮询间隔 5 秒，匹配范围 all，监控版块 all，规则 1 条（关键词：VPS），自检间隔 60 分钟');
  assert.equal(logs[1], '自检 RSS 连接正常');
  assert.equal(logs[2], '当前已是最新版本 v1.6.0');
...
test('启动自检 RSS 失败时推送降级自检通知', async () => {
  ...
  assert.deepEqual(events, [{ rssOk: false, version: '1.6.0', updateInfo: null }]);
});
